...
class params_couplings():
    ...
    gVd33 = 0.25
    gVu33 = 0.25
    ### Added vector couplings to leptons
    gVl11 = 0.25
    gVnu11 = 0.25
    gVl22 = 0.25
    gVnu22 = 0.25
    ...
    gAd33 = 0
    gAu33 = 0
    ### Added axial couplings to leptons
    gAl11 = 0
    gAnu11 = 0
    gAl22 = 0
    gAnu22 = 0
...
### Set the couplings
### Couplings determined for the benchmark model

ggauge = 0.1

couplings.gVXd = 0
couplings.gVd11 = -(1.0/4.0)*ggauge
couplings.gVu11 = -(1.0/4.0)*ggauge
couplings.gVd22 = -(1.0)*ggauge
couplings.gVu22 = 0
couplings.gVd33 = 0
couplings.gVu33 = 0
...
couplings.gVl22 = 0
couplings.gVnu22 = 0
couplings.gVl33 = +(3.0/4.0)*ggauge
couplings.gVnu33 = 0

couplings.gAXd = (3.0)*ggauge
couplings.gAd11 = +(1.0/4.0)*ggauge
couplings.gAu11 = +(3.0/4.0)*ggauge
couplings.gAd22 = -(1.0)*ggauge
couplings.gAu22 = 0
couplings.gAd33 = 0
couplings.gAu33 = 0
...
couplings.gAl22 = 0
couplings.gAnu22 = 0
couplings.gAl33 = -(3.0/4.0)*ggauge
couplings.gAnu33 = 0


### Function to calculate Z' width
def Zpr_width (couplings_struct, masses_struct):
    ...
    if (masses_struct.MY1 > 2 * masses_struct.MXd):
        partial_xi += masses_struct.MY1 / (12.0*math.pi) * math.sqrt(1 - 4*pow(masses_struct.MXd/masses_struct.MY1,2)) * (pow(couplings_struct.gVXd,2) * (1 + 2*pow(masses_struct.MXd/masses_struct.MY1,2)) + pow(couplings_struct.gAXd,2) * (1 - 4*pow(masses_struct.MXd/masses_struct.MY1,2)))
    
    ### partial width for each quark flavour (single color)
    # up quark
    partial_q += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVu11,2) + pow(couplings_struct.gAu11,2))
    # down quark
    partial_q += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVd11,2) + pow(couplings_struct.gAd11,2))
    # charm quark
    partial_q += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVu22,2) + pow(couplings_struct.gAu22,2))
    # strange quark
    partial_q += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVd22,2) + pow(couplings_struct.gAd22,2))
    # top quark
    if (masses_struct.MY1 > 2 * Mtop):
        partial_q += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVu33,2) + pow(couplings_struct.gAu33,2)) * pow(1 - 4 * pow(Mtop/masses_struct.MY1, 2), 3/2)
    # bottom quark
    if (masses_struct.MY1 > 2 * Mbottom):
        partial_q += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVd33,2) + pow(couplings_struct.gAd33,2)) * pow(1 - 4 * pow(Mbottom/masses_struct.MY1, 2), 3/2)
    
    ### partial width for each lepton flavour
    # electron
    partial_l += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVl11,2) + pow(couplings_struct.gAl11,2))
    # electron neutrino
    partial_l += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVnu11,2) + pow(couplings_struct.gAnu11,2))
    # muon
    partial_l += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVl22,2) + pow(couplings_struct.gAl22,2))
    # muon neutrino
    partial_l += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVnu22,2) + pow(couplings_struct.gAnu22,2))
    # tau
    if (masses_struct.MY1 > 2 * Mtau):
        partial_l += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVl33,2) + pow(couplings_struct.gAl33,2)) * pow(1 - 4 * pow(Mtau/masses_struct.MY1, 2), 3/2)
    # tau neutrino
    partial_l += masses_struct.MY1 / (12.0*math.pi) * (pow(couplings_struct.gVnu33,2) + pow(couplings_struct.gAnu33,2))
    
    return partial_xi + 3 * partial_q + partial_l


#### Set dummy masses and widths
...
parser = argparse.ArgumentParser(description="Monojet analysis script. Select the option 'analysis' to run the analysis (i.e. ./main_Bench2.py analysis), or the option 'plot' to produce the plot (i.e. ./main_Bench2.py plot)")
...
if (args.option == "analysis"):
    ...
    sys.exit("Choose between the option 'analysis' to run the analysis (i.e. ./main_Bench2.py analysis)\nand the option 'plot' to produce the plot (i.e. ./main_Bench2.py plot)")
...
if flag_run_analysis:
    ...
    for mZpr in mZpr_list:
        for mDM in mDM_list:            
            ...
            if (((mDM > 0.5*mZpr + 100) or (mDM < 0.5*mZpr - 150)) and (mZpr < 700)): continue ### skip points far from the on-shell limit
            #if ((mZpr >= 700) and (mDM > 0.5*mZpr)): continue ### for sufficiently heavy Zpr compute also small DM points, but do not go above on-shell limit
                
            masses.MY1 = mZpr
            masses.MXd = mDM
            
            ### Set the Z' width
            masses.WY1 = Zpr_width (couplings, masses) ### vector mediator calculated width
            ...
            if (mediator_type == 2):
                ...
                results.append([couplings.gVd11, couplings.gVu11, couplings.gAXd, masses.MY1, masses.MXd, float(format(xsection, ".2f"))])
